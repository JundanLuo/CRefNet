...
from solver import saw_utils
from utils.image_util import numpy_to_tensor, tensor_to_numpy
...
def eval_on_images(shading_image_arr, pixel_labels_dir, 
    ...
    img_path = os.path.join(img_dir, str(photo_id) + ".png")
...
def get_precision_recall_list_new(model, device,
                                  pixel_labels_dir, thres_list, photo_ids,
    ...
    for idx, photo_id in enumerate(photo_ids):
        if display_process and idx % 10 == 0:
        ...
        input_srgb = numpy_to_tensor(saw_img).unsqueeze(0).to(torch.float32).to(device)
        with torch.no_grad():
            pred = model(input_srgb=input_srgb, mode="IID")
        ...
        if idx % per_sample == 0:
            sample_arr.append({
                ...
                'pred_R': pred_R,
                'pred_S': pred_S,
                'input_srgb': input_srgb})
        pred_S_np = np.mean(tensor_to_numpy(pred_S[0]), axis=2)
        assert pred_S_np.shape == (original_h, original_w)
...
def compute_pr(model, device,
               pixel_labels_dir, splits_dir, dataset_split, class_weights, bl_filter_size, img_dir,
    ...
    # compute PR 
    rdic_list, sample_arr = get_precision_recall_list_new(model, device, pixel_labels_dir=pixel_labels_dir,
